...
import { Truck, Bell, Package } from '../../components/icons';
        ...
        <div className="p-6">
            ...
            <div className="mt-4 grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-4">
                {allProducts.slice(0, 4).map(product => (
                    <Link to={`/products/${product.id}`} key={product.id} className="border dark:border-gray-700 rounded-lg p-4 transition-shadow hover:shadow-md flex flex-col justify-between">
                        <div>
                           <h3 className="font-semibold text-gray-800 dark:text-gray-200">{product.title}</h3>
                           <p className="text-sm text-gray-500 dark:text-gray-400">{product.brand}</p>
                        </div>
                        <p className="text-lg font-bold text-gray-700 dark:text-gray-100 mt-4">${product.price}</p>
                    </Link>
                ))}
            </div>
        </div>
